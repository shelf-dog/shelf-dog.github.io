...
//		global.editor = Editor().initialise(global.container.empty());
		...
		// -- Set-Up Local Forage -- //
		global.cache = localforage.createInstance({
			name : "Shelf-Dog", version : 1.0,
			description : "Shelf Dog [Cache]",
		});
		// -- Set-Up Local Forage -- //
		...
		// -- Load GAPI & Picker (if required) -- //
		var token;
		if (token) {
    
      gapi.load("client:auth", {callback : function() {
           gapi.auth.setToken({
             access_token: token
           });
           gapi.load("picker", {callback : function() {
						 $("#gdrive-select").show().removeAttr("disabled").removeClass("dim").click(_googleDriveClick);
					 }});
        }
      });
